...
/* Spare fonts */
static char *font2[] = { "JoyPixels", "NotoColorEmoji" };
 ...
 */
static unsigned int su_timeout = 50;
 ...
 */
unsigned int defaultfg = 7; // 258;
unsigned int defaultbg = 0; // 259;
unsigned int defaultcs = 14; // 256;
static unsigned int defaultrcs = 15; // 257;
...
#define URL_SELECTOR_COMMAND(prefix) \
    "sed 's/.*│//g' | tr -d '\n' | " \
    "grep -oE '(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]*[-A-Za-z0-9+&@#/%=~_|]' | uniq | " \
    "sed 's/^www./http:\\/\\/www\\./g' | " \
    "rofi -dmenu -i -p '" prefix "' -l 10"

static char *openurlcmd[] = {
    "/bin/sh", "-c",
    URL_SELECTOR_COMMAND("Follow URL") " | xargs -r xdg-open",
    "externalpipe",
    NULL
};
...
static char *copyurlcmd[] = {
    "/bin/sh", "-c",
    URL_SELECTOR_COMMAND("Copy URL") " | xclip -selection clipboard",
    "externalpipe",
    NULL
};
